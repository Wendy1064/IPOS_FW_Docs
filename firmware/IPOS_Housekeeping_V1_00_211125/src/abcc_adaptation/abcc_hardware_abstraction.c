...
void HAL_SPI_TxRxCpltCallback( SPI_HandleTypeDef* pxHandle )
{
   //if( pxHandle == pxSPI_Handle )
	if (pxHandle->Instance == SPI2)
   {
      /* Deactivate SS, i.e. set it high. */
      HAL_GPIO_WritePin( ABCC_CSn_GPIO_Port, ABCC_CSn_Pin, GPIO_PIN_SET );
...
BOOL ABCC_HAL_Init( void )
{
   ...
   HAL_GPIO_WritePin( ABCC_RSTn_GPIO_Port, ABCC_RSTn_Pin, GPIO_PIN_RESET );
...
//   if( ( bOpmode & 0x1 ) != 0 )
//   {
//      HAL_GPIO_WritePin( ABCC_OM0_GPIO_Port, ABCC_OM0_Pin, GPIO_PIN_SET );
//   }
//   else
//   {
//      HAL_GPIO_WritePin( ABCC_OM0_GPIO_Port, ABCC_OM0_Pin, GPIO_PIN_RESET );
//   }
//
//   if( ( bOpmode & 0x2 ) != 0 )
//   {
//      HAL_GPIO_WritePin( ABCC_OM1_GPIO_Port, ABCC_OM1_Pin, GPIO_PIN_SET );
//   }
//   else
//   {
//      HAL_GPIO_WritePin( ABCC_OM1_GPIO_Port, ABCC_OM1_Pin, GPIO_PIN_RESET );
//   }
//
//   if( ( bOpmode & 0x4 ) != 0 )
//   {
...
{
   HAL_GPIO_WritePin( ABCC_RSTn_GPIO_Port, ABCC_RSTn_Pin, GPIO_PIN_SET );
}
...
{
   if( HAL_GPIO_ReadPin( ABCC_IRQ_GPIO_Port, ABCC_IRQ_Pin ) == GPIO_PIN_RESET )
   {
      return( TRUE );
   }
...
void ABCC_HAL_SpiSendReceive( void* pxSendDataBuffer, void* pxReceiveDataBuffer, UINT16 iLength )
{
   ...
   HAL_GPIO_WritePin( ABCC_CSn_GPIO_Port, ABCC_CSn_Pin, GPIO_PIN_RESET );
