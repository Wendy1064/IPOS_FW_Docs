...
/*------------------------------------------------------------------------------
** Local helpers: ANSI stripping + USB-backed printf wrappers
**----------------------------------------------------------------------------*/
...
/**
 * @brief Print to USB using a local buffer, stripping ANSI.
 *
 * Returns number of chars that *would* have been printed (like printf),
 * used for padding calculations.
 */
static int LogPrintf_ret(const char *fmt, ...)
{
#if ABCC_CFG_LOG_STRINGS_ENABLED
   char buf[256];
   va_list args;
...
/**
 * @brief Convenience wrapper when we don't care about return length.
 */
...
)
{
...
#if ABCC_CFG_LOG_TIMESTAMPS_ENABLED
   {
      ...
      LogPrintf( ABCC_LOG_ANSI_COLOR_GREEN"%02u:%02u:%02u.%03u - "ABCC_LOG_ANSI_COLOR_RESET,
...
#if ABCC_CFG_LOG_FILE_LINE_ENABLED
   {
      ...
      xLen = LogPrintf_ret( "%s:%d", pcBasename, xLine );
   ...
   LogPrintf( "%s", apcSeverityToString[ eSeverity ] );
...
#if ABCC_CFG_LOG_STRINGS_ENABLED
   {
      ...
      char buf[256];
      int len = vsnprintf(buf, sizeof(buf), pcFormat, argp);
      va_end( argp );
   ...
   {
      LogPrintf( "Error code: %d, Additional info: %" PRIu32 "\n", eErrorCode, lAdditionalInfo );
   }
   else
   {
      LogPrintf( "\n" );
...
void ABCC_LogMsg( ABP_MsgType* psMsg )
{
   ...
   LogPrintf( "%*sMsgBuf:0x%p Size:0x%04x SrcId:  0x%02x DestObj:0x%02x\n%*sInst:  0x%04x     Cmd: 0x%02x   CmdExt0:0x%02x CmdExt1:0x%02x\n",
   ...
   LogPrintf( "%*sData:  [ ", ABCC_LOG_INDENTATION, "" );
...
{
#if !ABCC_CFG_LOG_STRINGS_ENABLED
   LogPrintf( "%*s", ABCC_LOG_INDENTATION, "" );
#endif

   LogPrintf( "MsgBuf:0x%p SrcId:0x%02x\n",
              (void*)psMsg,
              ABCC_GetMsgSourceId( psMsg ) );
}
#endif /* ABCC_CFG_DEBUG_MESSAGING_ENABLED */
...
void ABCC_LogHexdumpMsg( ABP_MsgType* psMsg, BOOL fTx )
{
   ...
   while( iSizeInBytes > 0 )
   {
      ...
      LogPrintf( "%02x", (UINT8)(iData & 0xFF) );
      iSizeInBytes--;
      if( iSizeInBytes > 0 )
      {
         LogPrintf( "%02x", (UINT8)(( iData >> 8 ) & 0xFF) );
         iSizeInBytes--;
      }
   ...
   {
      LogPrintf( "%02x", ((UINT8*)psMsg)[ iIndex ] );
   }
#endif

   LogPrintf( "\n" );
}
#endif /* ABCC_CFG_DEBUG_HEXDUMP_MSG_ENABLED */
...
#endif /* ABCC_CFG_DEBUG_HEXDUMP_SPI_ENABLED */
...
#endif /* ABCC_CFG_DEBUG_HEXDUMP_UART_ENABLED */
